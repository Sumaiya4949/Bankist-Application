...
const account2 = {
  ...
  movementsDates: [
    ...
    '2021-09-11T18:49:59.371Z',
    '2021-09-10T12:01:20.894Z',
  ],
  currency: 'USD',
  locale: 'en-US',
};
...
/////////////////////////////////////////////////
// Functions
const calDisplayDate = function (dateData, locale) {
  const calcDaysPassed = (day1, day2) =>
    Math.round(Math.abs(day1 - day2) / (1000 * 60 * 60 * 24));

  const daysPassed = calcDaysPassed(new Date(), dateData);
  if (daysPassed === 0) return `Today`;
  if (daysPassed === 1) return `Yesterday`;

  return new Intl.DateTimeFormat(locale).format(dateData);
};
//currency fomatter
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const dateData = new Date(acc.movementsDates[i]);

    const displayDate = calDisplayDate(dateData, acc.locale);
